...
def reduce(S: np.array):
    coeffs = pso(S, 30, 2, 0.9, 20, 20, 0.7, 1.5, 1.5)
    L = int(coeffs[0])
    coeffs = coeffs[1:L+1]
    print(f"coeffs: {coeffs} lenght: {L} fitness: {fitness(S, coeffs, 0.2)}")
    raw_occ = find_occurrences(S, coeffs, 0.2)
    # merge_thresh = 2
    best_pattern = merge(raw_occ, L, 4)
    pattern = np.array(best_pattern).flatten()
    print(pattern)
    return pattern